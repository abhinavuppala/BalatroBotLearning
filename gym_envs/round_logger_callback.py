...
class RoundLoggerCallback(DefaultCallbacks):
    ...
    def on_train_result(
        ...
        **kwargs,
        ...
        stats = algorithm.env_runner_group.foreach_env_runner(func=get_and_reset_stats)
        ...
        # biases = algorithm.env_runner_group.foreach_env_runner(func=get_biases)
        ...
        for hand_type in hands:
            val = result[ENV_RUNNER_RESULTS]["custom_metrics"].get(
                f"hand_{hand_type}_mean", None
            )
            if val is not None:
                hand_means[hand_type] = val
        if hand_means:
        ...
        # algorithm.env_runner_group.foreach_env_runner(func=partial(_update_bias, bias=new_biases))
