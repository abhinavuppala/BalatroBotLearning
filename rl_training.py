...
# Reasonable defaults for Apple Silicon laptops.
CPU_CORES = os.cpu_count() or 8
DEFAULT_ENV_RUNNERS = min(4, max(1, CPU_CORES - 2))
...
def shop_config(model_config={}):
    ...
    config = (
        ...
        .env_runners(
            num_env_runners=DEFAULT_ENV_RUNNERS,
            num_envs_per_env_runner=1,
...
def blind_shop_config(
    ...
    config = (
        ...
        .callbacks(RoundLoggerCallback)
        .framework("torch")
        # IMPORTANT: Use the old (ModelV2/Policy/RolloutWorker) API stack.
        # The new RLModule stack tries to auto-build an encoder for our nested Dict
        # observation space, which triggers: "No default encoder config for obs space=Dict(...)"
        .api_stack(
            enable_rl_module_and_learner=False,
            enable_env_runner_and_connector_v2=False,
        )
        .resources(num_gpus=0, num_cpus_per_worker=1)  # , num_cpus_for_local_worker=10)
        .env_runners(
...
if __name__ == "__main__":
    ...
    ray.init(ignore_reinit_error=True, num_cpus=CPU_CORES)
    ...
    tune.run(
        ...
        config=config,
        name=training_mode,
        # resources_per_trial={"cpu": 8, "gpu": 0}, # cap resources per trial
        ...
        storage_path=os.path.join(os.getcwd(), "run_data"),
        trial_name_creator=lambda trial: f"{training_mode}_{trial.trial_id}",
...
# wandb key: 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